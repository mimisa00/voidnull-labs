...
- DB scripts at **root** are prefixed `prisma:`, **not `db:`** (no `db:*` at root). `apps/api/package.json` additionally defines `db:generate` / `db:migrate` / `db:seed` (each `cd ../..` then runs the root Prisma script) — they work, but the root `prisma:*` scripts are canonical.
- `npm test`'s glob is broken on Windows (`Invalid testPattern ... Running all tests instead`) and silently falls back to the full suite; it happens to match `jest.config.js` `testMatch`, so it passes.
...
- `npm run build` runs `nest build`, but root has neither `nest-cli.json` nor `tsconfig.json` (both only in `apps/api/`). **Verified 2026-08-24: it fails from root** (nest errors immediately on the missing tsconfig.json). Build from `apps/api`.
- No `typecheck` script. Closest is `npx tsc --noEmit -p apps/api/tsconfig.json` (**verified 2026-08-24: runs clean from root, zero type errors**).

## Repo layout & the broken workspace

Root `package.json` has **no `workspaces` field**; there is no `pnpm-workspace.yaml`. `turbo` and `next` are **not** in root `node_modules`. `turbo.json` exists but is a **dead file** — its task deps (`test dependsOn ^build`, `db:seed dependsOn db:migrate`) never run. `apps/api/node_modules` does not exist (it uses root deps); `apps/web` has its own. `apps/web/package.json` has a self-referential dep `"voidnull-labs": "file:../.."` — **verified 2026-08-24 to cause a recursive `node_modules` symlink chain pointing at the repo root**; as a result `git status --porcelain --ignored` hangs (~120s timeout with "Filename too long" noise), while plain `git status` is unaffected. **`packages/` directory was removed on 2026-08-12 (commits 005c328, c8cf0ba); all Prisma-related files (schema, migrations, seed) are now consolidated at repo root.**

- `apps/api` — all business logic (auth, users, rbac, game, gateway, redis, prisma, wallet, tournament, leaderboard, reports, approval). It also has its own `package.json` with `dev` and `db:generate` / `db:migrate` / `db:seed` scripts (all running against the repo root).
...
- Web auth token storage is **cookie-first, with localStorage mirrored alongside** (the old "four-file inconsistency" no longer exists as of 2026-08-24): `middleware.ts` reads the cookie **or** the `Authorization` header; the axios interceptor in `lib/api.ts` reads the cookie and, on refresh, writes cookie **and** localStorage; the login page writes cookie **and** localStorage; `use-auth.ts` is a thin wrapper over `auth-context.tsx`, which reads the cookie and base64-decodes the JWT itself. Read `middleware.ts`, `lib/api.ts`, `auth-context.tsx`, and the login page before changing anything auth-related.
...
- Constructor DI is written `private xxx: Service` (not `readonly`; single exception: `redis.service.ts` uses `private readonly`).
...
- **`console.log` is not the logging convention.** Leftover debug logs sit on production paths (`auth.service.ts` logs the whole user object, `auth.controller.ts` logs the login body, three `layout.tsx` files under `apps/web/src/app` log `[WS] notification`). That's tech debt. NestJS `Logger` is used in `app.gateway.ts` and `game.service.ts`.
...
- Jest currently runs **11 suites / 119 tests** under `apps/api/` (verified 2026-08-24 with no services up — all pass; the glob in `jest.config.js` is authoritative, don't copy a fixed count here). Coverage: the two guard specs (`test/guards/permissions.guard.spec.ts`, `roles.guard.spec.ts`), plus wallet, leaderboard, tournament, game-state-machine, `game.service`, `blackjack.engine`, `users.service`, `auth.service`, and `game.controller`.
...
- `.husky/pre-commit` is **active**: `npx lint-staged` then `npm test --silent`. **Every commit runs the full test suite (~48s as of 2026-08-24 — 11 suites / 119 tests).** No commitlint.
- Remote `github.com/mimisa00/voidnull-labs` (`https://github.com/mimisa00/voidnull-labs.git`). **`main` is the only remote branch** — README's `develop`/`staging` do not exist; locally there is additionally an un-checked-out worktree branch `feature/my-first-ai-task-for-test-worktree`. Commit messages are loose Conventional Commits (`feat:`/`fix:`/`chore:`/`docs:`/`refactor:`, no scopes); many have no prefix. English.
- `CLAUDE.md` and the old `AGENTS.md` were intentionally deleted. Do not recreate `CLAUDE.md`.

## Do not touch / security

`infra/docker/certbot/conf/` holds **real Let's Encrypt certificates and private keys** for `staging.voidnull.io` (`privkey.pem`, account `private_key.json`) on local disk. They are **not tracked by git** and are correctly excluded by `.gitignore` (line 32 — the `infra/docker/certbot/conf/` directory entry covers both keys) — keep it that way. `infra/docker/certbot/cloudflare.ini` (untracked, `.gitignore:34`) and real `.env` files are also on disk. Never read, echo, print, or commit their contents, and never `git add -f` them. If a task requires them, stop and tell the user.

## Known broken or disabled

- **`game` module is fully wired** (this doc previously claimed "half-removed / WebSocket does not" — stale as of 2026-08-24): `app.gateway.ts` imports `GameService` and its `game:create` / `game:join` / `game:action` handlers call real `gameService` methods; `GameController` exposes full REST CRUD; the blackjack UI lives in `apps/web/src/app/games/blackjack/`. **Both REST and WebSocket are implemented.**
- **`ThrottlerModule` is commented out in `app.module.ts`** — there is currently **no rate limiting**, despite `docs/spec/spec.md:225` claiming 60 requests/minute.
...
- References to `@docs/guideline/code-standards.md` mean `docs/guideline/code-standards.md` (there is no `@docs/` directory).
- `docs/guideline/architecture_spec.md` is mojibake-corrupted (UTF-8/Big5 mix) and partly unreadable — don't quote it as a spec.
- Artifacts: `.playwright-mcp/`, `.qa-artifacts/`, `test-results/`. `test-results/` **is now gitignored** (`.gitignore:46`, verified 2026-08-24); the directory appears after Playwright runs (not on disk as of 2026-08-24).

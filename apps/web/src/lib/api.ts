...
export const authApi = {
  ...
  verifyTotp: (tempToken: string, code: string) => api.post('/auth/2fa/verify', { tempToken, code }).then((r) => r.data),
  ...
  enableTotp: (code: string) => api.post('/auth/2fa/enable', { code }).then((r) => r.data),
};

export const usersApi = {
  list: (page = 1, limit = 20) => api.get(`/users?page=${page}&limit=${limit}`).then((r) => r.data),
  get: (id: string) => api.get(`/users/${id}`).then((r) => r.data),
  create: (data: any) => api.post('/users', data).then((r) => r.data),
  update: (id: string, data: any) => api.put(`/users/${id}`, data).then((r) => r.data),
  delete: (id: string) => api.delete(`/users/${id}`),
  assignRoles: (id: string, roleIds: string[]) => api.put(`/users/${id}/roles`, { roleIds }).then((r) => r.data),
};
...
export const rolesApi = {
  ...
  assignPermissions: (id: string, permissionIds: string[]) => api.put(`/roles/${id}/permissions`, { permissionIds }).then((r) => r.data),
};
